...
const NoticiasCurso = ({ idCurso }) => {
    ...
    useEffect(() => {
        ...
        const fetchCurso = async () => {
          try {
            const response = await fetch(`https://centroformacion.hopto.org/api/posts?filters[curso][id][$eq]=${idCurso}&populate=*`);

            const data = await response.json();
            
            setPosts(data.data)

            // Asignamos el curso correctamente desde data.data
    ...
    <div className='container mx-auto p-4 '>
        ...
        <div className="border-b mb-5 flex justify-between text-sm">
                <div className="text-indigo-600 flex items-center pb-2 pr-2 border-b-2 border-indigo-600 uppercase">
                    ...
                    <a href="#" className="font-semibold inline-block">Este curso</a>
                </div>
       ...
       posts.map((post) => {
          ...
          return (
            ...
            <div className="relative"><a href="#">
                ...
                <div href="#!">
                    <div
                        className="text-xs absolute top-0 right-0 bg-indigo-600 px-2 py-1 text-white mt-3 mr-3 hover:bg-white hover:text-indigo-600 transition duration-500 ease-in-out">
                          {post.attributes.curso.data.attributes.Nombre}
                    </div>
                </div>
            </div>
            <div className="px-6 py-4 mb-auto">
                <a href="#"
                    className="font-medium text-base hover:text-indigo-600 transition duration-500 ease-in-out inline-block mb-2">
                    {post.attributes.Titulo}</a>
                <p className="text-gray-500 text-sm">
                            {post.attributes.descripcionCorta }
                </p>
            </div>
            <div className="px-6 py-3 flex flex-row items-center justify-between bg-gray-100">
                <span href="#" className="py-1 text-xs font-regular text-gray-900 mr-1 flex flex-row items-center">
                    ...
                    <span className="ml-1">{month} {day}</span>
                </span>
