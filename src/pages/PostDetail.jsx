...
const PostDetail = () => {
    ...
    useEffect(() => {
        // Función para obtener el post por ID
        const fetchPost = async () => {
          try {
            const response = await fetch(`https://centroformacion.hopto.org/api/posts/${id}?populate=*`);
            const data = await response.json();
            setPost(data.data); // Asignamos el post correctamente desde data.data
      ...
      // Dividimos el texto en párrafos usando dos saltos de línea
      if(!post){
        console.log("noexiste")
      }
      else{
        const paragraphs = post.attributes.Contenido.split("\n\n"); // Dividimos el texto en párrafos usando dos saltos de línea
        
        setParrafos(paragraphs)
...
<div className="border-b mb-5 flex justify-between text-sm mt-8">
    <div className="text-indigo-600 flex items-center pb-2 pr-2 border-b-2 border-indigo-600 uppercase">
      ...
      <div className="descripcion lg:ml-28 lg:mr-28 mt-10">
        
      
        <ReactMarkdown>{post.attributes.Contenido}</ReactMarkdown>
